...
def network_exist(ssid, bssid):
	...
	try:
		...
		if not output:
			print( Fore.YELLOW+"=> No Wi-Fi networks found by nmcli."+ Style.RESET_ALL)
			return False
		...
		print(Fore.BLUE+"DEBUG: Network not found."+ Style.RESET_ALL)
		return False

	except subprocess.CalledProcessError as e:
		print(Fore.RED + f"=> Error fetching Wi-Fi list: {e.stderr.strip()}"+ Style.RESET_ALL)
		return False
	except FileNotFoundError:
		print(  Fore.YELLOW+"=> Error: nmcli command not found. Please ensure NetworkManager is installed."+ Style.RESET_ALL)
		return False
...
def list_available_networks():
	...
	try:
		...
		print( Fore.RED+f"=> Error listing Wi-Fi networks: {e.stderr}"+ Style.RESET_ALL)
		print( Fore.LIGHTYELLOW_EX+"	Please ensure NetworkManager is running and you have appropriate permissions."+ Style.RESET_ALL)
	except FileNotFoundError:
		print( Fore.LIGHTYELLOW_EX+ "=>  Error: 'nmcli' command not found. Please ensure NetworkManager is installed."+ Style.RESET_ALL)
	except Exception as e:
		print( f"=>  An unexpected error occurred while listing networks: {e}"+ Style.RESET_ALL)
...
def connect_with_profile(ssid, bssid,path="/usr/share/wordlists/rockyou.txt"): 
	...
	try:
		...
		print( Fore.YELLOW+f"=>  Error: Password file not found at '{path}'. Please ensure the path is correct."+ Style.RESET_ALL)
		return
	except Exception as e:
		print( Fore.RED + f"=>  Error reading password file: {e}"+ Style.RESET_ALL)
		return

	if not passwords:
		print(Fore.YELLOW+f" > Warning: No passwords found in the file '{path}'."+ Style.RESET_ALL)
		return
	print("\n-------------------------------------------------------------------------------------------------------")
	...
	for i, password in enumerate(passwords):
		...
		try:
			...
			print( Fore.LIGHTYELLOW_EX+ f"=>  Connection attempt timed out after 20 seconds."+ Style.RESET_ALL)
			continue
...
"""
	...
	path_input = input(Fore.LIGHTWHITE_EX +" > Enter path to password file (Press Enter to use default rockyou.txt): "+ Style.RESET_ALL)
	...
	if not ssid or not bssid:
		...
		print( Fore.RED+f"=>  The network '{ssid}' with BSSID '{bssid}' is not currently visible."+Style.RESET_ALL)
